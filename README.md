...
local function C(p, r) local c=Instance.new("UICorner",p) c.CornerRadius=UDim.new(0,r or 12) end
...
sBox.Position         = UDim2.new(0, 12, 0, 28)
...
C(sBox, 9)
S(sBox, STROKE)
local pad1=Instance.new("UIPadding",sBox) pad1.PaddingLeft=UDim.new(0,12)
...
kRow.Position         = UDim2.new(0, 12, 0, 28)
...
local kLbl = makeLabel(kRow, FlyKey.Name, 12, 0, 240, 34, 15, WHITE, true)
...
makeLabel(track, "Disabled", 14, 0, 150, 34, 13, GRAY)
...
UserInputService.InputBegan:Connect(function(inp, gpe)
    ...
    if inp.KeyCode==FlyKey then if FlyOn then stopFly() else startFly() end end
end)
